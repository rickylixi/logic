...
\begin{frame}\frametitle{MDL vs Bayesian Mixture}
	\begin{center}
		\resizebox{0.9\textwidth}{!}{\hspace{1.5em}
				\xymatrix @C=1pc{&&&*+++[o][F--]{\txt{\textit{\textbf{\underline{\textcolor{yellow}{{\text{Model}}}}}}}}&&\\
					&&&&
					\ar@2{-->}^*++[o]{\textcolor{red}{\text{\footnotesize Deduction}}} "1,4";"5,7"
					&&&\\
					\ar@2{-->}^*++[o]{\textcolor{red}{\text{\footnotesize Induction}}} "5,1";"1,4"
					&&&&&\\
					\ar@2{->}^*++[o]{\textcolor{red}{\text{\footnotesize Prediction}}} "5,1";"5,7"\\
					*+++[o][F--]{\txt{\textit{\textbf{\underline{\textcolor{yellow}{{\text{Sample}}}}}}}}&&&&&&*+++[o][F--]{\txt{\textit{\textbf{\underline{\textcolor{yellow}{{\text{Data}}}}}}}}}}
	\end{center}
...
\begin{frame}\frametitle{Bayesian Decisions}
	...
	\end{center}
	\[
	y_t^{\Lambda_\rho}(x_{<t}) \;:=\; \arg\min\limits_{y_t}\sum\limits_{x_t}\rho(x_t|x_{<t})\operatorname{Loss}(x_t,y_t)
	\]
	\begin{align*}
	&L^{\Lambda_\rho}(x_{<t}) \;:=\; \mathbb{E}_\mu\left[\operatorname{Loss}\left(x_t,y_t^{\Lambda_\rho}\right)\,\middle|\, x_{<t}\right]\\
...
\begin{frame}\frametitle{Solution~$2$ --- ad hoc}
	\[\textcolor{yellow}{w(\theta):=\dfrac{1}{2}(1+\delta(1-\theta))}\]
...
\begin{frame}\frametitle{Indifference Principle}
	...
	\[p(n,r):=\left|\left\{(m_0,\ldots,m_n): \sum\limits_{i=1}^n i\cdot m_i=n\;\;\&\;\;\sum\limits_{i=0}^n m_i=r\;\;\&\;\;\forall i: m_i\geq 0 \right\}\right|\]
...
\begin{frame}\frametitle{(B) Structure Description~$\checkmark$}
	...
	\begin{center}
		\textcolor{yellow}{Sometimes we write $m^*(h_{1:n}):=m^*\left(\bigwedge\limits_{i=1}^n Q_{h_i}(a_i)\right)$ for short.}
	\end{center}
\end{frame}
...
\begin{frame}\frametitle{No Free Lunch Theorem --- Strengthened ``Hume''}
	\begin{theorem}[No Free Lunch Theorem]
		...
		where $T_n:=\left\langle(x_1,f(x_1)),\dots,(x_n,f(x_n))\right\rangle$, $T_n^x:=\left\langle x_1,\dots,x_n\right\rangle$, $T_n^y:=\left\langle f(x_1),\dots,f(x_n)\right\rangle$ and $A: T_n\mapsto x_{n+1}\in\mathcal{X}\setminus T_n^x$.
	\end{theorem}
...
\begin{frame}\frametitle{No-Free-Lunch for Pure Inductive Logic}
	\begin{itemize}
		...
		\item Take $\mathcal{X}:=\mathcal{C}, \mathcal{Y}:=\mathcal{Q}$ in the No-Free-Lunch Theorem. The state description $h:\mathcal{C}\to\mathcal{Q}$ can be taken as a \emph{classification} function.
...
\begin{frame}\frametitle{Time Series and Solomonoff Induction}
	\begin{itemize}
		...
		\begin{align*}
		M'(\epsilon)&:=1\\
		M'(h_{1:n})&:=M'(h_{<n})\frac{M(h_{1:n})}{\sum\limits_{Q\in\mathcal{Q}}M(h_{<n}Q)}
		\end{align*}
...
\setlength\belowdisplayskip{0pt}
	...
	\end{definition}
	\resizebox{\textwidth}{!}{PAC-learnable under $\Delta:=\left\{\mathcal{D}:\mathcal{D}\leqm\xi\right\}\iff$ PAC-learnable under $\xi(x):=\sum\limits_{\nu\in\mathcal{M}}2^{-K(\nu)}\nu(x)$.}
\end{frame}
...
$\operatorname{VC}(\mathcal{H}):=\sup\big\{|C|: \mathcal{H} \mbox{ shatters } C\big\}$.
\end{definition}
		...
		\column{0.7\textwidth}
			\begin{figure}[htbp]
				\begin{center}
					...
					\node[circle, draw, text centered, minimum height=2em, fill=darkgray!50!green!50] (a1) {$s_1$};
					\node[circle, draw, text centered, minimum height=2em, right of=a1, fill=darkgray!50!green!50] (e1) {$s_2$};
					\node[circle, draw, text centered, minimum height=2em, right of=e1, fill=darkgray!50!green!50] (a2) {$s_3$};
					\node[circle, draw, text centered, minimum height=2em, right of=a2, fill=darkgray!50!green!50] (e2) {$s_4$};
			...
			\begin{figure}[htbp]
				\begin{center}
					\begin{tikzpicture}[node distance=17mm, auto]
					\node[circle, draw, text centered, minimum height=2em, fill=red!50!darkgray] (a1) {$a_1$};
					\node[circle, draw, text centered, minimum height=2em, right of=a1, fill=orange!50!gray] (e1) {$e_1$};
					\node[circle, draw, text centered, minimum height=2em, right of=e1, fill=red!50!darkgray] (a2) {$a_2$};
					\node[circle, draw, text centered, minimum height=2em, right of=a2, fill=orange!50!gray] (e2) {$e_2$};
					\node[right of=e2,minimum height=2em] (dots) {\ldots};
					\node[circle, draw, text centered, minimum height=2em, above of=a1, fill=green!50!darkgray] (hidden) {$h$};
					\draw[->] (hidden) to (a1);
					\draw[->] (hidden) to (e1);
					\draw[->] (hidden) to (a2);
					\draw[->] (hidden) to (e2);
...
\begin{enumerate}
	\item Let $f:\mathcal{X}\to 2^{<\omega}$ be uniquely decodable. Let $\ell_x:=\ell(f(x))$. Then $f$ satisfies the Kraft inequality
...
\item $M'\left(A(\vec{a})\right):=\sum\limits_{\Theta(\vec{b})\vDash A(\vec{a})} M'(\Theta(\vec{b}))$\\
where
...
where $A(a_{1:t}):=\bigwedge\limits_{i=1}^t A(a_i/x)$.
\end{frame}
...
\begin{frame}\frametitle{All Ravens are Black!~$\checkmark$}
	...
	\begin{theorem}[Confirmation by Random Sampling]
		If the sampling function $t:\mathbb{N}\to\mathbb{N}$ satisfies $\forall i: t_i\leq t_{i+1}$ and $\chi_{1:\infty}$ is Martin-L\"of random, where $\chi_i:=\llbracket\exists k(t_k=i)\rrbracket$, then
...
\begin{frame}\frametitle{Universal Portfolios}
	\begin{itemize}
		\item the agent chooses a distribution $\mathbf{b}_t\in\Delta_n:=\left\{\mathbf{x}\in[0,1]^n:\|\mathbf{x}\|_1=1\right\}$ of wealth over $n$ goods.
		...
		\item regret.
		$R_t:=\max\limits_{\mathbf{b}\in\Delta_n}\sum\limits_{k=1}^t\log \mathbf{b}_k^\top(\mathbf{x}_{<k})\mathbf{x}_k-\sum\limits_{k=1}^t\log \mathbf{b}_k^\top(\mathbf{x}_{<k})\mathbf{x}_k$
		\item universal portfolios.
		\begin{align*}
		\hat{\mathbf{b}}_1&:=\left(\tfrac{1}{n},\dots,\tfrac{1}{n}\right)\\
		\hat{\mathbf{b}}_{t+1}(\mathbf{x}_{1:t})&:=\frac{\int_{\Delta_n}\!\!\mathbf{b} W_t(\mathbf{b},\mathbf{x})\mathrm{d}\mathbf{b}}{\int_{\Delta_n}\!\!W_t(\mathbf{b},\mathbf{x})\mathrm{d}\mathbf{b}}
		\end{align*}
...
\begin{itemize}
	\item For $x\in 2^{<\omega}$, the cylinder set $\Gamma_x:=\{y\in 2^\omega: x\prec y\}$ is the basic open set. It corresponds to the interval $[0.x,0.x+2^{-\ell(x)})$.
	...
	\item The outer measure of $C\subset 2^\omega$ is $\mu^*(C):=\inf\left\{\sum\limits_{x\in A}2^{-\ell(x)}: C\subset\Gamma_A\right\}$.
...
\begin{itemize}
	\item A total lower semicomputable function $\delta: 2^{<\omega}\to\omega$ is a Martin-Löf test if $\forall n:\mu(V_n)\leq 2^{-n}$, where $V_n:=\{x:\delta(x)\geq n\}$.
...
\end{center}
\[R_b:=\big\{x\in 2^\omega:\exists n\big(K(x_{1:n})<n-b\big)\big\}\]
\begin{center}
...
\[\operatorname{depth_b}(x):=\min\left\{t: U^t(p)=x\;\;\&\;\;\ell(p)-K(x)\leq b\right\}\]
\end{definition}
...
\begin{frame}\frametitle{Effective Complexity}
	...
	\begin{minipage}{\textwidth}
				...
				\beta_x(k)&:=\min\limits_A\{\delta(x|A): x\in A\;\;\&\;\;K(A)\leq k\} &\text{[Best-Fit]}\\
				h_x(k)&:=\min\limits_A\{\log |A|: x\in A\;\;\&\;\;K(A)\leq k\} &\text{[Kolmogorov structure function / ML]}\\
				h_x(k)&:=\min\limits_\mu\{-\log \mu(x): K(\mu)\leq k\} &\text{[[Kolmogorov structure function / ML]}\\
				A^*(x)&:=\iota A\Big[x\in A\;\;\&\;\;K(A)=\mu k\big[k+h_x(k)\eqa K(x)\big]\Big]&\text{[Kolmogorov minimal sufficient statistic]}\\
				\lambda_x(k)&:=\min\limits_A\left\{K(A)+\log |A|: x\in A\;\;\&\;\;K(A)\leq k\right\} &\text{[MDL]}\\
				\lambda_x(k)&:=\min\limits_\mu\{K(\mu)-\log\mu(x): K(\mu)\leq k\} &\text{[MDL]}\\
				\Delta(x|A)&:=K(A)+\log |A|-K(x) &\text{[discrepancy]}\\
				\operatorname{soph_c}(x)&:=\min\limits_A\{K(A):\Delta(x|A)<c\} &\text{[sophistication]}\\
				\operatorname{csoph}(x)&:=\min\limits_A\{K(A)+\Delta(x|A)\} &\text{[coarse sophistication]}\\
				\Sigma(\mu)&:=K(\mu)+H(\mu) &\text{[total information]}\\
				\mathcal{E}_{\delta,\Delta}(x|\mathcal{M})&:=\min\limits_{\mu\in\mathcal{M}}\left\{K(\mu):\Sigma(\mu)-K(x)\leq\Delta\;\;\&\;\;\mu(x)\geq 2^{-H(\mu)(1+\delta)}\right\} &\text{[effective complexity]}\\
				\mathcal{E}_\delta(x|\mathcal{M})&:=\min\limits_{\mu\in\mathcal{M}}\left\{K(\mu)+\Sigma(\mu)-K(x): \mu(x)\geq 2^{-H(\mu)(1+\delta)}\right\} &\text{[coarse effective complexity]}
				\end{align*}
	\end{minipage}}
\end{frame}
...
\end{theorem}
\begin{align*}
K^t(x)&:=\min\limits_p\{\ell(p): U^t(p)=x\}\tag{\text{\footnotesize Time-bounded Kolmogorov Complexity}}\\
\operatorname{depth^t}(x)&:=K^t(x)-K(x)\tag{\text{\footnotesize Basic Computational Depth}}\\
\operatorname{depth_{BB}}(x)&:=\min\limits_t\{\operatorname{depth^t}(x)+K(t)\}\tag{\text{\footnotesize Busy Beaver Computational Depth}}
\end{align*}
\begin{theorem}
...
\[\mbox{advantage: }\quad A^\pi(s,a):=Q^\pi(s,a)-V^\pi(s)\]
\end{frame}
...
\begin{frame}\frametitle{Application of Banach's Fixpoint Theorem}
	\begin{theorem}
		...
		$\mathcal{V}:=\left\{V\in\mathbb{R}^{\mathcal{S}}: \|V\|_\infty<\infty\right\}$ and $\|V\|_\infty:=\max\limits_{s\in\mathcal{S}}|V(s)|$.
	\end{theorem}
	\begin{block}{}
		\begin{itemize}
...
\begin{frame}\frametitle{Universal Approximation Theorem}
	\begin{theorem}[Universal Approximation Theorem]
		...
		where
		\[h(\mathbf{x}):=\sum\limits_{i=1}^{N} v_i g\left(\mathbf{w}_i^\top\mathbf{x} + b_i\right)\]
...
\begin{frame}\frametitle{Deep Learning}\vspace{-2ex}
	...
	\begin{enumerate}
		...
		\item \textcolor{green}{pick the best function} --- Learn --- \textcolor{yellow}{find the network parameters $\theta^*:=\argmin\limits_\theta L(\theta)$ that minimize total cost $L(\theta)$ by gradient decent} \[\textcolor{yellow}{\theta\gets\theta-\eta\nabla_\theta L(\theta)}\]		
		\textcolor{yellow}{where $L(\theta):=\mathbb{E}_P\left[\ell\left(f_\theta(\mathbf{a}),t \right)\right]+\lambda\Omega(\theta)$ and $\Omega(\theta)$ is a regularizer.}
	\end{enumerate}
\end{frame}
...
\setlength\belowdisplayskip{0pt}
	...
	\end{center}
	where parameters $\theta:=\left\{\mathbf{w}^{(i)},\mathbf{b}^{(i)}\right\}_{i=1}^n$ and activation functions $\mathbf{g}$
...
\xymatrix{*++{current\atop player} \ar@//[r] & *++[F-]{\mbox{policy evaluation}\atop\mbox{\textcolor{yellow}{neural network}}} \ar@//[rrr]^{\small \mbox{\!\!\!\!\!\!\!\!\!\!\!position `value'}\atop\mbox{\!\!\!\!\!\!\!\!\!\!\!move `probability'}} &&& *++[F-]{\mbox{policy improvement}\atop\mbox{\textcolor{yellow}{neural network}}} \ar@//[r] & *++{improved\atop player} \ar@/^4pc/[lllll]_{\mbox{self-learning/policy iteration}}}
\end{minipage}}
\begin{columns}
\column{.49\textwidth}
	\begin{figure}
...
\end{itemize}
\[\fbox{$T^*:=\argmin\limits_{T: I(T(X);Y)=I(X;Y)}I(X;T(X))$}\] is the \textcolor{yellow}{Information Bottleneck} between $X$ and $Y$.
\end{frame}
		...
		\begin{center}
			\resizebox{.95\textwidth}{!}{\begin{minipage}{70ex}
						\xymatrix @C=1pc{&&&&&&&\\
							...
							&*+++[o]{\txt{\textit{\textbf{\underline{\large{\textcolor{red}{Agent}}}}}}}
							&&&&*+++[F=]{\txt{\textit{\textbf{\underline{\Huge\textcolor{yellow}{{Environment}}}}}}}
	...
	\begin{center}
		\boxed{
			\begin{minipage}{60ex}
				...
				\textcolor{yellow}{\mathrm{AIXI}}&:=\argmax\limits_\pi\Upsilon(\pi)=\pi_\xi^*
				\end{align*}
		\end{minipage}}
	\end{center}
	\[V_\xi^\pi(h)=\sum\limits_{\nu\in\mathcal{M}}w_h^\nu V_\nu^\pi(h)\]
	\[\textcolor{yellow}{w_\nu:=2^{-K(\nu)}}\implies\xi(e_{1:m}|a_{1:m})\eqm M(e_{1:m}|a_{1:m}):=\textcolor{yellow}{\sum\limits_{p:U(p,a_{1:m})=e_{1:m}}\!\!2^{-\ell(p)}}\]
\end{frame}
...
\setlength\belowdisplayskip{0pt}
	...
	\[\left\langle\Pi:=\mathcal{A}^{(\mathcal{A}\times\mathcal{E})^*}, D(\pi,\pi'):=\mathrm{e}^{-\min\left\{n:\exists h_{<n}\left(\pi(h_{<n})\neq\pi'(h_{<n})\right)\right\}}\right\rangle\]\vspace{-2ex}
	\begin{center}
...
\setlength\abovedisplayskip{0pt}
\setlength\belowdisplayskip{0pt}
	\[\Gamma_t:=\sum\limits_{i=t}^\infty\gamma_i\qquad H_t(\varepsilon):=\min\left\{m: \dfrac{\Gamma_{t+m}}{\Gamma_t}\leq\varepsilon\right\}\]
	\begin{theorem}
...
\begin{frame}\frametitle{Intrinsic Utility}
	\begin{itemize}
		...
		\item \textcolor{green}{information gain} $\textcolor{yellow}{H(w_{h_{<t}})-H(w_{h_{1:k}})}$ where \[H(w_h):=-\sum\limits_{\nu\in\mathcal{M}}w_h^\nu\log w_h^\nu\]
...
\[w_\epsilon^\nu\mapsto\bar{U}(\nu)\mapsto w_\epsilon^\nu\]
\[
\pi^*:=\argmax\limits_\pi\mathbb{E}_\xi^\pi\left[\sum\limits_{t=1}^\infty u^{\mathrm{in}}(h_{1:t})\right]
\]
\end{frame}
...
\begin{frame}\frametitle{\href{http://www.hutter1.net/publ/ksaprob.pdf}{Knowledge-Seeking Agent}}\vspace{-2ex}
	\[
	V_{\mathrm{IG}}^{\pi,m}(h_{<t}):=\mathbb{E}_\xi^\pi\left[\textcolor{yellow}{H\big(w_{h_{<t}}^\cdot\big)-H\big(w_{h_{1:m}}^\cdot\big)}\,\middle|\, h_{<t}\right]=\sum\limits_{\nu\in\mathcal{M}} w_{h_{<t}}^\nu D_m\big({}_\nu^\pi\big\|{}_\xi^\pi\bigm\vert h_{<t}\big)
	\]
	\[
	\begin{aligned}
	...
	V_{\mathrm{IG}}^\pi(h_{<t})&:=\mathbb{E}_\xi^\pi\left[\sum\limits_{k=t}^\infty\gamma_k \textcolor{yellow}{D\big(w_{h_{1:k}}^\cdot\big\| w_{h_{<k}}^\cdot\big)}\,\middle|\, h_{<t}\right]=\sum\limits_{\nu\in\mathcal{M}}w_{h_{<t}}^\nu D_\gamma\big({}_\nu^\pi\big\|{}_\xi^\pi\bigm\vert h_{<t}\big)\\
	\pi_{\mathrm{IG}}^*&:=\argmax\limits_\pi V_{\mathrm{IG}}^\pi
	\end{aligned}
	\]
...
\setlength\belowdisplayskip{0pt}
\begin{align*}
V_{\mathrm{IG}}^\pi(h_{<t})&:=\mathbb{E}_\xi^\pi\left[D\big(w_{h_{<t+m}}^\cdot\big\| w_{h_{<t}}^\cdot\big)\,\middle|\, h_{<t}\right]\\
\pi_{\mathrm{IG}}^{m,k}&:=\argmax_{\pi\in\mathcal{A}^{\mathcal{H}^{<m}}} V_{\mathrm{IG}}^\pi(h_{<t-k})\\
\rho(h_{<t},m,k)&:=\min\left\{\frac{1}{m^2(m+1)}, \eta V_{\mathrm{IG}}^{\pi_{\mathrm{IG}}^{m,k}}(h_{<t-k})\right\}
\end{align*}
\begin{algorithm}[H]
\begin{algorithmic}[1]
\While{True}
...
\end{algorithm}
\resizebox{\textwidth}{!}{$\pi^\dagger(a|h_{<t}):=\sum\limits_{m\in\mathbb{N}}\sum\limits_{k<m,t}\rho(h_{<t},m,k)\left\llbracket a=\pi_{\mathrm{IG}}^{m,k}(h_{<t})\right\rrbracket+\left(1-\sum\limits_{m\in\mathbb{N}} \sum\limits_{k<m,t}\rho(h_{<t},m,k)\right)\left\llbracket a=\pi_\xi^*(h_{<t})\right\rrbracket$}
\end{frame}
...
\begin{frame}\frametitle{Approximation}
	...
	\begin{itemize}
		...
		\[\Phi: h\mapsto s\]
		\[\Phi^{\mathrm{best}}:=\argmin\limits_\Phi \operatorname{Cost}(\Phi|h)\]
		\[\operatorname{Cost}(\Phi|h):=\operatorname{CL}(s_{1:n}^\Phi|a_{1:n})+\operatorname{CL}(r_{1:n}|s_{1:n}^\Phi,a_{1:n})+\operatorname{CL}(\Phi)\]
...
\begin{itemize}
	...
	\[\operatorname{CTW}(e_{<t}|a_{<t}):=\sum\limits_\Gamma 2^{-\operatorname{CL}(\Gamma)}\Gamma(e_{<t}|a_{<t})\]
	\item \textcolor{yellow}{Select} actions with highest upper confidence bound.
\[a_{\mathrm{ucb}}:=\argmax\limits_{a\in\mathcal{A}}\left(\underbrace{\hat{Q}(\ae_{<t}a)}_{\text{average}}+\underbrace{\sqrt{\frac{\log T(\ae_{<t})}{T(\ae_{<t}a)}}}_{\text{exploration bonus}}\right)\]
...
\setlength\belowdisplayskip{0pt}
	...
	\end{block}\vspace{-2ex}
	\[
	Kt(x):=\min\limits_p\big\{\ell(p)+\log t(p,x): U(p)=x\big\}
	\]\vspace{-2ex}
	\begin{theorem}
	...
	\begin{columns}
		\column{.5\textwidth}
			\begin{enumerate}
				...
				\item[] \textcolor{red}{Initialize the shared variables $L:=\{\},\;\; t_{\mathrm{fast}}:=\infty,\;\; p_{\mathrm{fast}}:=p^*$.}
		...
		\column{.5\textwidth}
			\begin{enumerate}
				...
				\item[] \hspace{-1em}\textcolor{orange}{{for}} \textcolor{green}{$i:=1,2,3,\dots$} \textcolor{orange}{{do}}
	...
	\begin{columns}
		\column{.5\textwidth}
			\begin{enumerate}
				...
				\item[] \textcolor{orange}{then} \textcolor{yellow}{$t_{\mathrm{fast}}:=t(x)$ and $p_{\mathrm{fast}}:=p$.}
				\item[] $\hspace{-1em}$\textcolor{orange}{continue}
			\end{enumerate}
		\column{.5\textwidth}
			\begin{enumerate}
				\item[] {\hspace{-1em}\hspace{-1em} \textcolor{cyan}{\fbox{$C$}}}
				\item[] \hspace{-1em}\textcolor{orange}{for} \textcolor{cyan}{$k:=1,2,4,8,\dots$} \textcolor{orange}{do}
...
\begin{enumerate}
	...
	\[\textcolor{yellow}{\operatorname{VA}(p):=``\forall k\forall(va^\prime\ae)_{1:k}: p(\ae_{<k})=v_1a_1^\prime\dots v_ka_k^\prime\implies v_k\leq V_\xi^\pi(\ae_{<k})"}\]
	\textcolor{green}{(The program $p$ not only computes future actions of $\pi$, which is the policy derived from $p$ according to $\pi(\ae_{<k}):=a_k^\prime$, but also hypothetical past actions $a_i^\prime$ and lower bounds $v_i$ for the value of the policy $\pi$.)}
...
An arrow $f: X\times X\to Y$ is \emph{weakly point surjective} if for every $g: X\to Y$, there is a $t: \mathbf{1}\to X$ such that, for all $x: \mathbf{1}\to X$:
...
\setlength\belowdisplayskip{0pt}
	\begin{theorem}[Lawvere's Fixpoint Theorem]
		For sets $X, Y, Z$, functions $\beta: X\to Y, f: X\times Y\to Z, \alpha: Z\to Z$, let $g:=\alpha\circ f\circ(1_X,\beta)$. Assume \textcolor{red}{$\beta$ is surjective}. 
		\begin{enumerate}
			\item If \textcolor{yellow}{$\alpha$ has no fixpoint}, then \textcolor{green}{$g$ is not representable by $f$}.
			\item If \textcolor{yellow}{$g$ is representable by $f$}, then \textcolor{green}{$\alpha$ has a fixpoint}.
		\end{enumerate}
...
\begin{frame}\frametitle{Non-operational Self-inspection?}
\begin{quote}
...
\begin{frame}\frametitle{\href{http://www.vetta.org/documents/Machine_Super_Intelligence.pdf}{Incompressibility vs Incompleteness vs Intelligence}}
	\begin{columns}
		\column{\textwidth}
			\begin{itemize}
				\item $P(x):=\big\{p\in\mathcal{X}^*:\exists m\forall n\geq m\left(p(x_{1:n})=x_{n+1}\right)\big\}$
				\item $P(A):=\bigcap\limits_{x\in A}P(x)$
				\item $P_n:=P\big(\big\{x: Km(x)\leq n\big\}\big)$
...
\end{enumerate}
\[\resizebox{\textwidth}{!}{$u(h):=\left\llbracket K(h)<\ell(h)\;\;\&\;\;\forall h^\prime\succ h\bigg(K(h)\leq K(h^\prime)\;\;\&\;\;\forall \operatorname{part}(h)\Big(\sum\limits_{h^\prime\in \operatorname{part}(h)}K(h^\prime)\geq K(h)\Big)\bigg)\right\rrbracket$}
\]
\end{frame}
...
\begin{itemize}
	...
	\item search for models $y_i^*:=\argmin\limits_{y: S(y)=x_i}\ell(y)$ for each $x_i$ w.r.t. some best representation $S^*:=\argmin\limits_S\left[\ell(S)+\sum\limits_{i=1}^n\ell(y_i^*)\right]$
\end{itemize}
\begin{enumerate}
...
\[S':=\operatorname{spec}(\operatorname{MSearch},S)\implies
\begin{cases}
\forall x: S(S'(x))=x\\
